...
class NextEngineClient
{
	...
	*/
	public function __construct() {
		...
		////////////////////////////////////////////////////////////////////////////
		$this->_client_id		= config('nextengine.client_id');//$client_id ;
		$this->_client_secret	= config('nextengine.client_secret') ;

		///////////////////////////////////////////////////////
		// SDK内部でAPIを利用する為に使うメンバ変数
		///////////////////////////////////////////////////////
		// 認証パラメータ
		$this->_redirect_uri  = config('nextengine.redirect_uri') ;
		$this->_access_token  = config('nextengine.access_token') ;
		$this->_refresh_token = config('nextengine.refresh_token') ;
		$this->_verifypeer    = config('nextengine.verifypeer');
		$this->_redirect_uri  = config('nextengine.redirect_uri') ;

		curl_setopt($this->_curl, CURLOPT_HEADER,			false) ;
		curl_setopt($this->_curl, CURLOPT_ENCODING,			'Accept-Encoding: gzip,deflate') ;
		curl_setopt($this->_curl, CURLOPT_SSL_VERIFYPEER,	$this->_verifypeer) ;
		...
		if(!is_null($this->_redirect_uri)) {
			curl_setopt($this->_curl, CURLOPT_REFERER,		'https://'.parse_url($this->_redirect_uri, PHP_URL_HOST)) ;
		}
	...
	*/
	public function apiExecute($path, $api_params = array(), $redirect_uri = NULL) {
		...
		// APIを実行して処理結果を返す
		$response = $this->post(static::getApiServerHost().$path, $api_params) ;
	...
	*/
	public function login($redirect_uri = NULL) {
		...
		$params = array('uid' => $this->_uid, 'state' => $this->_state, 'client_id' => $this->_client_id, 'client_secret' => $this->_client_secret) ;
	...
	protected static function getApiServerHost(){
		return self::API_SERVER_HOST;
	}

	protected static function getNeServerHost(){
	...
	*/
	...
	protected function redirectLogin() {
		...
		$url = static::getNeServerHost().self::PATH_LOGIN.'?'.$this->getUrlParams($params) ;
